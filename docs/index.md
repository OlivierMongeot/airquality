...
Des messages d'alertes sont créées en fonction des changements des données, les déclenchements sont paramétrables individuellement.
...
1. Par ville  : vous rentrez le nom d'une ville et son code pays, vous testez si l'API reconnait bien cette ville, puis vous enregistrez.
...
En cliquant sur les moyennes, vous accédez au graphique représentant ces données (fonction native de Jeedom)
...
Ce plugin est simplement informatif, il ne remplace en aucun cas d'éventuelles indications données par votre médecin ou tout autre corps médical.
